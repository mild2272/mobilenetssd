...
lineaccesstoken = 'lQ0TWa+lk+B+Rkd+0vvW8iLruNEFJjgs8PUe10ay84ev2+IUin2hKWiBlBeC+92xVoDMirEtvL9KUHTF8ZgUnKUodmrHuyzfuaTfTJPUbvtAwcodlCK8EcpbflcFf88LurUtT5viKx+Fw80xyvmkcwdB04t89/1O/w1cDnyilFU='
...
def event_handle(event):
    ...
    if msgType == "text":
        ...
        if msg == "ทำอะไรอยู่" :
            replyObj = TextSendMessage(text="คุยกับนายไง")
        if msg == "ดูหนังกันมั้ย" :
            replyObj = TextSendMessage(text="นายจ่ายนะ")
        else :  
            replyObj = TextSendMessage(text="อะไรว่ะ")
        line_bot_api.reply_message(rtoken, replyObj)
    elif msgType == "image":
        try:
